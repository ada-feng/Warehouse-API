...
class inventory_client():
    ...
    @staticmethod
    def request_body_formatter(body_dic):
...
class TestAPIResponse(unittest.TestCase):
    ...
    def testGetExistItem(self):
        setup_dic = {"a": "6"}
        setup_dic_formatted = self.iv_client.request_body_formatter(setup_dic)
        self.iv_client.httpmethod("PUT", self.path, setup_dic_formatted)
        request_body_dic = {'name': 'a'}
        request_body_formatted = self.iv_client.request_body_formatter(request_body_dic)
    ...
    def testGetIllegalBounds(self):
        ...
        = self.iv_client.request_body_formatter(lowerHigherThanUpper_dic)
        status, response_body  =  self.iv_client.httpmethod('get', self.path, \
        lowerHigherThanUpper_dic_formatted)
        self.assertEqual(status, 200 )
        self.assertEqual(response_body, {} )
        ...
        = self.iv_client.request_body_formatter(notInteger_dic)
        with self.assertRaises(Exception):
